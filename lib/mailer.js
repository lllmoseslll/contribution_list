...
import { getSettings, getNotifications, saveNotifications, formatUGX } from './budget-service';
...
export async function sendPledgeNotifications(pledge, itemInfo, updatedStats) {
  const settings = getSettings();
  const notifications = getNotifications();

  // Where pledge alerts go. Separate from ownerEmail, which is the committee's
  // public contact address — one field cannot serve two purposes with opposite
  // disclosure requirements. ownerEmail remains the fallback so nothing breaks
  // before a recipient is chosen.
  const notifyEmail = settings.notifyEmail || settings.ownerEmail || process.env.OWNER_EMAIL || null;
        ...
        <div class="content">
          ...
          <table class="details-table">
            ...
            <tr>
              ...
              <td class="val" style="color:#16a34a; font-weight:700;">${itemRemaining}</td>
            </tr>
            <tr>
