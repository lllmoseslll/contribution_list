...
export async function sendPledgeNotifications(pledge, itemInfo, updatedStats, spilloverInfo = null) {
  ...
  // Where pledge alerts go — only addresses the committee explicitly added
  // via "Add Recipient" on the Settings tab and saved. notifyEmail (a
  ...
  // Deliberately does NOT fall back to ownerEmail or OWNER_EMAIL: those are
  // the public "Contact Email" shown to contributors, a separate, display-only
  // setting — opting an inbox into real-time pledge alerts must be an explicit
  // action, never an inferred one.
  ...
  // pledge.amount only ever holds the item-covered portion when a pledge
  // spills over — the contributor's real total is itemAmount + spilloverAmount.
  const generalFundAmount = spilloverInfo?.spilloverAmount || 0;
  const itemAppliedAmount = spilloverInfo ? spilloverInfo.itemAmount : pledge.amount;
  const totalPledgedAmount = itemAppliedAmount + generalFundAmount;

  const formattedAmount = formatUGX(totalPledgedAmount);
      ...
      <div class="container">
        ...
        <div class="content">
          ...
          <table class="details-table">
            ...
            ${generalFundAmount > 0 ? `
              <tr>
                <td class="label">Applied to ${itemName}:</td>
                <td class="val" style="font-weight:700;">${formatUGX(itemAppliedAmount)}</td>
              </tr>
              <tr>
                <td class="label">General Fund Excess:</td>
                <td class="val" style="color:#047857; font-weight:700;">+ ${formatUGX(generalFundAmount)} to General Ceremony Fund</td>
              </tr>
            ` : ''}
            <tr>
          ...
          <body style="font-family:sans-serif; color:#1e293b; padding:20px;">
            <div style="max-width:550px; margin:auto; border:1px solid #cbd5e1; border-radius:10px; padding:24px;">
              ...
              <div style="background:#ecfdf5; border:1px solid #10b981; border-radius:8px; padding:12px; margin:16px 0;">
                ...
                ${generalFundAmount > 0 ? `
                  <p style="margin:4px 0;">${itemName} was fully covered by <strong>${formatUGX(itemAppliedAmount)}</strong> of your pledge — the remaining <strong>${formatUGX(generalFundAmount)}</strong> was added to the General Ceremony Fund. Thank you for your extra generosity!</p>
                ` : ''}
              </div>
