...
export async function buildPledgeReportPdf({ forAdmin = false } = {}) {
  ...
  doc.font('Helvetica-Bold').fontSize(8).fillColor(ACCENT)
    .text('STILL NEEDED', boxX + 16, boxY + 10, { width: halfW - 32 });
  doc.font('Helvetica-Bold').fontSize(22).fillColor(BRAND_DARK)
    .text(formatUGX(stats.totalRemaining), boxX + 16, boxY + 22, { width: halfW - 16 });

  doc.font('Helvetica-Bold').fontSize(8).fillColor(BRAND)
    .text('RAISED SO FAR', boxX + halfW, boxY + 10, { width: halfW - 16, align: 'right' });
  doc.font('Helvetica-Bold').fontSize(22).fillColor(BRAND)
    .text(`${stats.totalPercentage}%`, boxX + halfW, boxY + 22, { width: halfW - 16, align: 'right' });
  ...
  doc.text('SECTION', secCols.name, y, { width: secCols.target - secCols.name - 6 });
  doc.text('TARGET', secCols.target, y, { width: secCols.remaining - secCols.target - 6 });
  doc.text('REMAINING', secCols.remaining, y, { width: secCols.pct - secCols.remaining - 6 });
  doc.text('% FUNDED', secCols.pct, y, { width: PAGE_WIDTH - PAGE_MARGIN - secCols.pct });
  y += 11;
  ...
  sections.forEach((sec) => {
    ...
    doc.font('Helvetica').fontSize(8.5).fillColor(NEUTRAL)
      .text(`${sec.code}: ${sec.title}`, secCols.name, y, { width: secCols.target - secCols.name - 6 });
    doc.fillColor(NEUTRAL).text(formatUGX(sec.totalCost), secCols.target, y, { width: secCols.remaining - secCols.target - 6 });
    doc.fillColor(sec.remainingAmount > 0 ? ACCENT : BRAND).text(formatUGX(sec.remainingAmount), secCols.remaining, y, { width: secCols.pct - secCols.remaining - 6 });
    doc.fillColor(NEUTRAL_LIGHT).text(`${sec.percentage}%`, secCols.pct, y, { width: PAGE_WIDTH - PAGE_MARGIN - secCols.pct });
    y += 13;
  });
