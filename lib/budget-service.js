...
export async function addNotification(notif) {
  ...
  const { rows } = await query(
    `INSERT INTO notifications
       (id, type, recipient, recipient_name, pledger_name, pledger_phone, amount, item, status, error, html_preview)
    ...
    [
      ...
      notif.status || 'recorded', notif.error || null, notif.htmlPreview || null
    ]
  );
